...
def run_pass(left_team,right_team,x,y):
    ...
    for i in range(len(right_team)):
        if right_team[i][0] > x and y +.01 >= right_team[i][1] and right_team[i][1]>= y - .01 and \
           abs(right_team[i][0] - x) <.01:
    ...
    for i in range(len(left_team)):
        ...
        if left_team[i][0] >= x and left_team[i][1] < y and abs(left_team[i][1] - x) <.05:
            teammate_left += 1
    ...
    if x >.75 and (teammate_left > 0 or teammate_right > 0):
